...
BevolkMITdeut <- read_excel("BevolkMITdeut.xlsx")
...
- adjust Variable "AnErfFaelle", "AnVersuche" and "AnAufkl". 
In this case we consider the NUmber of the Straftaten in Relation to the number of the Population per Bundesland
```{r}
BevolkMITdeut <- BevolkMITdeut[,c(1,3,4)]
LF <- Reduce(left_join,  list(LF, BevolkMITdeut))
LF <- LF%>%mutate(AnErfFaelle = AnErfFaelle /BevölkAnzahl*100, 
           AnVersuche = AnVersuche/BevölkAnzahl*100, 
           AnAufkl = AnAufkl/BevölkAnzahl*100)
#check
view(LF)
sum(is.na(LF$AnErfFaelle))

```

- adjust values of the Kriminalitätsvariable, for the years 2015  and 2016.

```{r}
### each Row get a fix ID
LF$IDrow <- 1:nrow(LF)

### select the interesting variable, the interesting Straftaten for the years  2015, 2016
Straf72 <- c('------','000000', '100000', '200000', '****00', '500000' , '600000', '700000') 
Jahr1417 <- c('2014', '2015', '2016', '2017')
LFadj <- LF%>%select( IDrow, IDStraftat, Bundesland, Jahr,AnErfFaelle, AnVersuche, TeilVersuche, AnAufkl, TeilAufkl)
LFadj<- subset(LFadj, IDStraftat %in% Straf72 & Jahr %in% Jahr1417)

### reorder
LFadj <- LFadj%>%arrange(IDStraftat, Bundesland, Jahr)

### create a new variable which set the data for the years 2014 till 2017 to NA 
LFadj$AnErfFaelleADJ <- LFadj$AnErfFaelle
LFadj$AnErfFaelleADJ <- ifelse(LFadj$Jahr > 2014 & LFadj$Jahr < 2017,  LFadj$AnVersucheADJ == NA, LFadj$AnErfFaelleADJ)

LFadj$AnVersucheADJ <- LFadj$AnVersuche
LFadj$AnVersucheADJ <- ifelse(LFadj$Jahr > 2014 & LFadj$Jahr < 2017,  LFadj$AnVersucheADJ == NA, LFadj$AnVersucheADJ)

LFadj$TeilVersucheADJ <- LFadj$TeilVersuche
LFadj$TeilVersucheADJ <- ifelse(LFadj$Jahr > 2014 & LFadj$Jahr < 2017,  LFadj$TeilVersucheADJ == NA, LFadj$TeilVersucheADJ)

LFadj$AnAufklADJ <- LFadj$AnAufkl
LFadj$AnAufklADJ <- ifelse(LFadj$Jahr > 2014 & LFadj$Jahr < 2017,  LFadj$AnAufklADJ == NA, LFadj$AnAufklADJ)

LFadj$TeilAufklADJ <- LFadj$TeilAufkl
LFadj$TeilAufklADJ <- ifelse(LFadj$Jahr > 2014 & LFadj$Jahr < 2017,  LFadj$TeilAufklADJ == NA, LFadj$TeilAufklADJ)

### function for calculationg the interpolation over the years 2014 till 2017 on the variable AnErFaelle
i = 1
j = 4
while (j <= nrow(LFadj)) {
 x <- LFadj[i:j, 10]
 aprox <- na.approx(x)
 LFadj[i+1, 10] <- aprox[2]
 LFadj[i+2, 10] <- aprox[3]
 i = i +4
 j = j +4
}

### function for calculationg the interpolation over the years 2014 till 2017 on the variable AnVersuche
i = 1
j = 4
while (j <= nrow(LFadj)) {
 x <- LFadj[i:j, 11]
 aprox <- na.approx(x)
 LFadj[i+1, 11] <- aprox[2]
 LFadj[i+2, 11] <- aprox[3]
 i = i +4
 j = j +4
}

### function for calculationg the interpolation over the years 2014 till 2017 on the variable Teilversuche
i = 1
j = 4
while (j <= nrow(LFadj)) {
 x <- LFadj[i:j, 12]
 aprox <- na.approx(x)
 LFadj[i+1, 12] <- aprox[2]
 LFadj[i+2, 12] <- aprox[3]
 i = i +4
 j = j +4
}

### function for calculationg the interpolation over the years 2014 till 2017 on the variable AnAufkl
i = 1
j = 4
while (j <= nrow(LFadj)) {
 x <- LFadj[i:j, 13]
 aprox <- na.approx(x)
 LFadj[i+1, 13] <- aprox[2]
 LFadj[i+2, 13] <- aprox[3]
 i = i +4
 j = j +4
}

### function for calculationg the interpolation over the years 2014 till 2017 on the variable TeilAufkl
i = 1
j = 4
while (j <= nrow(LFadj)) {
 x <- LFadj[i:j, 14]
 aprox <- na.approx(x)
 LFadj[i+1, 14] <- aprox[2]
 LFadj[i+2, 14] <- aprox[3]
 i = i +4
 j = j +4
}
...
LFadj <- LFadj%>%select( IDrow,AnErfFaelleADJ, AnVersucheADJ, TeilVersucheADJ, AnAufklADJ, TeilAufklADJ)
LFadj2 <- Reduce(left_join, list(LF,LFadj)) 

### check
select(LFadj2,IDStraftat, Bundesland, Jahr,AnErfFaelle,AnErfFaelleADJ, AnVersuche,AnVersucheADJ, TeilVersuche, TeilVersucheADJ, AnAufkl, AnAufklADJ, TeilAufkl, TeilAufklADJ)%>%filter(Jahr %in% c(2013, Jahr1417, 2019), IDStraftat %in% '700000')%>%arrange(IDStraftat, Bundesland, Jahr)%>%view()

### replace the values
LFadj2$AnErfFaelleADJ <- ifelse(is.na(LFadj2$AnErfFaelleADJ), LFadj2$AnErfFaelle, LFadj2$AnErfFaelleADJ)
LFadj2$AnVersucheADJ <- ifelse(is.na(LFadj2$AnVersucheADJ), LFadj2$AnVersuche, LFadj2$AnVersucheADJ)
LFadj2$TeilVersucheADJ <- ifelse(is.na(LFadj2$TeilVersucheADJ), LFadj2$TeilVersuche, LFadj2$TeilVersucheADJ)
LFadj2$AnAufklADJ <- ifelse(is.na(LFadj2$AnAufklADJ), LFadj2$AnAufkl, LFadj2$AnAufklADJ)
LFadj2$TeilAufklADJ <- ifelse(is.na(LFadj2$TeilAufklADJ), LFadj2$TeilAufkl, LFadj2$TeilAufklADJ)

### check
select(LFadj2,IDStraftat, Bundesland, Jahr,AnErfFaelle,AnErfFaelleADJ, AnVersuche,AnVersucheADJ, TeilVersuche, TeilVersucheADJ, AnAufkl, AnAufklADJ, TeilAufkl, TeilAufklADJ)%>%filter(Jahr %in% c(2013, Jahr1417, 2018, 2019), IDStraftat %in% '700000')%>%arrange(IDStraftat, Bundesland, Jahr)%>%view()

###rename the variables 
LF <- LFadj2
colnames(LF)
LF <- select(LF, -c("AnErfFaelle", "AnVersuche", "TeilVersuche","AnAufkl", "TeilAufkl"))
LF <- dplyr::rename(LF, AnErfFaelle = AnErfFaelleADJ, 
                  AnVersuche = AnVersucheADJ,
                  TeilVersuche = TeilVersucheADJ,
                  AnAufkl = AnAufklADJ, 
                  TeilAufkl = TeilAufklADJ)
```
...
Save the Dataframe so far

```{r}
saveRDS(LF, file="3LF.rds")
saveRDS(LSO, file="3LSO.rds")
```
Reload the DataFrames
```{r}

#LSO <- readRDS("3LSO.rds")
#LF <- readRDS("3LF.rds")
...
GG0.1 <- ggplot(DS0, aes(Jahr, AnErfFaelle))+geom_line()+ylim(4000000, 7000000)+ ggtitle("Entwicklung der Straftaten insgesamt in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt")+theme(plot.title = element_text(size = 14))
GG0.1
GG0.2 <- ggplot(DS0, aes(Jahr, AnVersuche))+geom_line()+ylim(350000, 550000)+ ggtitle("Entwicklung der Versuche insgesamt in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt")+theme(plot.title = element_text(size = 14))
GG0.2
GG0.3 <- ggplot(DS0, aes(Jahr, TeilVersuche)) +geom_line()+ylim(0, 10)+ ggtitle("Entwicklung der Versuche insgesamt in Deutschland") +  xlab("Jahr") + ylab("Anteil der Versuche insgesamt")+theme(plot.title = element_text(size = 14))
GG0.3
GG0.4 <- ggplot(DS0, aes(Jahr, AnAufkl))+geom_line()+ylim(3000000, 4000000)+ ggtitle("Entwicklung der aufgeklärten Fälle insgesamt in Deutschland") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle")+theme(plot.title = element_text(size = 14))
GG0.4
GG0.5 <- ggplot(DS0, aes(Jahr, TeilAufkl)) +geom_line()+ylim(0, 100)+ ggtitle("Entwicklung der aufgeklärten Fälle insgesamt in Deutschland") +  xlab("Jahr") + ylab("Anteil der aufgeklärten Fälle insgesamt")+theme(plot.title = element_text(size = 14))
GG0.5
```
...
GG1.1 <- ggplot(DS1.1, aes(x=Jahr, y=AnErfFaelle, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Straftaten in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt")+theme(plot.title = element_text(size = 14))
GG1.1
DS1.2 <- LF%>%filter(Straftat == "Straftaten insgesamt" & Bundesland != "Deut")
GG1.2 <- ggplot(DS1.2, aes(x=Jahr, y=AnErfFaelle, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Straftaten in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt")+theme(plot.title = element_text(size = 14))
GG1.2
GG1.3 <- ggplot(DS1.2, aes(x=Jahr, y=AnErfFaelle, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Straftaten in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt")+theme(plot.title = element_text(size = 14))+ theme(legend.position="bottom")+ scale_colour_discrete(name="Verabschiedung des Gesetzes im Jahr", breaks=c("0", "1", "2","3", "4"),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"))
GG1.3
GG1.4 <- ggplot(DS1.2, aes(x=Jahr, y=AnErfFaelle, colour = TG_KG, group=Bundesland))+ geom_line()  +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt") + theme(plot.title = element_text(size = 14)) +scale_colour_manual(values = c('red','gray62','gray62','seagreen3', 'seagreen4'),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"), name="Verabschiedung des Gesetzes im Jahr")+ ggtitle("Entwicklung der Straftaten in den Bundesländern")+ theme(legend.position="bottom")
GG1.4 
...
GG7.1<- ggplot(DS7.1, aes(x=Jahr, y=AnErfFaelle, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der Straftaten in Bayern") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG7.1
DS7.2 <- DS7%>%filter(Bundesland =="Nordrhein-Westfalen")
GG7.2<- ggplot(DS7.2, aes(x=Jahr, y=AnErfFaelle, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der Straftaten in NRW") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen") 
GG7.2

### for Bayern more details 
DS7.3 <- LF%>%filter(StraftatGruppe == "Strafrecht.Nebenges" & Bundesland =="Bayern" & Jahr == "2015")
mostCommBayStraf <-  top_n(DS7.3, 15, AnErfFaelle)
mostCommBayStrafID <- mostCommBayStraf$IDStraftat
DS7.4 <- LF%>%filter(StraftatGruppe == "Strafrecht.Nebenges" & Bundesland =="Bayern")
out <- subset(DS7.4, IDStraftat %in% mostCommBayStrafID)
GG7.4<- ggplot(out, aes(x=Jahr, y=AnErfFaelle, colour = IDStraftat, group=IDStraftat))+ geom_line()+ ggtitle("Entwiklung der häufigsten Strafrechtliche Nebengesetze in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt  ")
GG7.4
```
...
GG2.1 <- ggplot(DS2.1, aes(x=Jahr, y=AnVersuche, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Versuche in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt")+theme(plot.title = element_text(size = 14))
GG2.1

DS2.2 <- LF%>%filter(Straftat == "Straftaten insgesamt" & Bundesland != "Deut")
GG2.2 <- ggplot(DS2.2, aes(x=Jahr, y=AnVersuche, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Versuche in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt")+theme(plot.title = element_text(size = 14))
GG2.2
GG2.3 <- ggplot(DS2.2, aes(x=Jahr, y=AnVersuche, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Versuche in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt")+theme(plot.title = element_text(size = 14))+ theme(legend.position="bottom")+ scale_colour_discrete(name="Verabschiedung des Gesetzes im Jahr", breaks=c("0", "1", "2","3", "4"),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"))
GG2.3
GG2.4 <- ggplot(DS2.2, aes(x=Jahr, y=AnVersuche, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Versuche in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt")+theme(plot.title = element_text(size = 14))+scale_colour_manual(values = c('red','gray62','gray62','seagreen3', 'seagreen4'),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"), name="Verabschiedung des Gesetzes im Jahr")+ theme(legend.position="bottom")
GG2.4
```
...
GG3.1 <- ggplot(DS3.1, aes(x=Jahr, y=TeilVersuche, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung des Versuchsanteils in den Bundesländern") +  xlab("Jahr") + ylab("Versuchsanteil insgesamt")+theme(plot.title = element_text(size = 14))
GG3.1

DS3.2 <- LF%>%filter(Straftat == "Straftaten insgesamt" & Bundesland != "Deut")
GG3.2 <- ggplot(DS3.2, aes(x=Jahr, y=TeilVersuche, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung des Versuchsanteils in den Bundesländern") +  xlab("Jahr") + ylab("Versuchsanteil insgesamt")+theme(plot.title = element_text(size = 14))
GG3.2
GG3.3 <- ggplot(DS3.2, aes(x=Jahr, y=TeilVersuche, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung des Versuchsanteils in den Bundesländern") +  xlab("Jahr") + ylab("Versuchsanteil insgesamt")+theme(plot.title = element_text(size = 14))+ theme(legend.position="bottom")+ scale_colour_discrete(name="Verabschiedung des Gesetzes im Jahr", breaks=c("0", "1", "2","3", "4"),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"))
GG3.3
GG3.4 <- ggplot(DS3.2, aes(x=Jahr, y=TeilVersuche, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung des Versuchsanteils in den Bundesländern") +  xlab("Jahr") + ylab("Versuchsanteil insgesamt")+theme(plot.title = element_text(size = 14))+scale_colour_manual(values = c('red','gray62','gray62','seagreen3', 'seagreen4'),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"), name="Verabschiedung des Gesetzes im Jahr")+ theme(legend.position="bottom")
GG3.4
```
...
GG4.1 <- ggplot(DS4.1, aes(x=Jahr, y=AnAufkl, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwiklung der aufgeklärten Fälle in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt")+theme(plot.title = element_text(size = 14))
GG4.1

DS4.2 <- LF%>%filter(Straftat == "Straftaten insgesamt" & Bundesland != "Deut")
GG4.2 <- ggplot(DS4.2, aes(x=Jahr, y=AnAufkl, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der aufgeklärten Fälle in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt")+theme(plot.title = element_text(size = 14))
GG4.2
GG4.3 <- ggplot(DS4.2, aes(x=Jahr, y=AnAufkl, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der aufgeklärten Fälle in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt")+theme(plot.title = element_text(size = 14))+ theme(legend.position="bottom")+ scale_colour_discrete(name="Verabschiedung des Gesetzes im Jahr", breaks=c("0", "1", "2","3", "4"),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"))
GG4.3
GG4.4 <- ggplot(DS4.2, aes(x=Jahr, y=AnAufkl, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der aufgeklärten Fälle in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt")+theme(plot.title = element_text(size = 14))+scale_colour_manual(values = c('red','gray62','gray62','seagreen3', 'seagreen4'),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"), name="Verabschiedung des Gesetzes im Jahr")+  theme(legend.position="bottom")
GG4.4
```
...
GG5.1 <- ggplot(DS5.1, aes(x=Jahr, y=TeilAufkl, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Aufklärungsquote in den Bundesländern") +  xlab("Jahr") + ylab("Aufklärungsquote insgesamt")+theme(plot.title = element_text(size = 14))
GG5.1

DS5.2 <- LF%>%filter(Straftat == "Straftaten insgesamt" & Bundesland !="Deut")
GG5.2 <- ggplot(DS5.2, aes(x=Jahr, y=TeilAufkl, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Aufklärungsquote in den Bundesländern") +  xlab("Jahr") + ylab("Aufklärungsquote insgesamt")+theme(plot.title = element_text(size = 14))
GG5.2
GG5.3 <- ggplot(DS5.2, aes(x=Jahr, y=TeilAufkl, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Aufklärungsquote in den Bundesländern") +  xlab("Jahr") + ylab("Aufklärungsquote insgesamt")+theme(plot.title = element_text(size = 14))+ theme(legend.position="bottom")+ scale_colour_discrete(name="Verabschiedung des Gesetzes im Jahr", breaks=c("0", "1", "2","3", "4"),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"))
GG5.3
GG5.4 <- ggplot(DS5.2, aes(x=Jahr, y=TeilAufkl, colour = TG_KG, group=Bundesland))+ geom_line()+ ggtitle("Entwicklung der Aufklärungsquote in den Bundesländern") +  xlab("Jahr") + ylab("Aufklärungsquote insgesamt")+theme(plot.title = element_text(size = 14))+scale_colour_manual(values = c('red','gray62','gray62','seagreen3', 'seagreen4'),labels=c("2018 (Bayern)", "2018", "2019", "2020", "2021"), name="Verabschiedung des Gesetzes im Jahr")+  theme(legend.position="bottom")
GG5.4
...
DS6.1 <- DS6%>%filter(IDStraftat =="------"| 
                      ...
                      IDStraftat =="600000"| 
                      IDStraftat =="700000")
GG6.1.1 <- ggplot(DS6.1, aes(x=Jahr, y=AnErfFaelle, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der Straftaten in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung","Straftaten insgesamt", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.1.1

GG6.1.2 <- ggplot(DS6.1, aes(x=Jahr, y=AnVersuche, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der Versuche in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Versuche insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung","Straftaten insgesamt", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.1.2

GG6.1.3 <- ggplot(DS6.1, aes(x=Jahr, y=TeilVersuche, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung des Versuchsanteils in Deutschland") +  xlab("Jahr") + ylab("Versuchenanteil der Straftaten insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Straftaten insgesamt","Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.1.3

GG6.1.4 <- ggplot(DS6.1, aes(x=Jahr, y=AnAufkl, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der aufgeklärten Fälle in Deutschland") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung","Straftaten insgesamt", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.1.4

GG6.1.5 <- ggplot(DS6.1, aes(x=Jahr, y=TeilAufkl, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung der Aufklärungsquote in Deutschland") +  xlab("Jahr") + ylab("Aufklärungsquelle insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Straftaten insgesamt", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.1.5
...
DS6.2  <- DS6%>%filter(IDStraftat =="000000"|  
                      ...
                      IDStraftat =="600000"| 
                      IDStraftat =="700000")
GG6.2.1 <- ggplot(DS6.2, aes(x=Jahr, y=AnErfFaelle, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ggtitle("Entwiklung der Straftaten in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Straftaten insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.2.1

GG6.2.2 <- ggplot(DS6.2, aes(x=Jahr, y=AnVersuche, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ggtitle("Entwicklung der Versuche in Deutschland") +  xlab("Jahr") + ylab("Anzahl der Vesuche insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.2.2

GG6.2.3 <- ggplot(DS6.2, aes(x=Jahr, y=TeilVersuche, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ggtitle("Entwicklung des Versuchsanteils in Deutschland") +  xlab("Jahr") + ylab("Versuchenanteil insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.2.3

GG6.2.4 <- ggplot(DS6.2, aes(x=Jahr, y=AnAufkl, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ggtitle("Entwicklung der aufgeklärten Fälle in Deutschland") +  xlab("Jahr") + ylab("Anzahl der aufgeklärten Fälle insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.2.4

GG6.2.5 <- ggplot(DS6.2, aes(x=Jahr, y=TeilAufkl, colour = StraftatGruppe, group=IDStraftat))+ geom_line()+ ggtitle("Entwicklung Aufklärungsquote in Deutschland") +  xlab("Jahr") + ylab("Aufklärungsquote insgesamt ")+theme(plot.title = element_text(size = 14))+scale_colour_discrete(labels=c("Diebstahl", "sonst. Straftatbestände", "Straftaten gegen das Leben", "strafrechtliche Nebengesetze", "Straftaten gegen die persönliche Freiheit", "Straftaten gegen die sexuelle Selbstbestimmung", "Vermögens- und Fälschungsdelikte"), name="Straftatenobergruppen")
GG6.2.5
```
...
GG8.1<- ggplot(DS8, aes(x=Jahr, y=AnErfFaelle, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwiklung der Strafrechtliche Nebengesetze in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Strafrechtliche Nebengesetze")+theme(plot.title = element_text(size = 14))
GG8.1
### overview on Diebstahl
DS8 <- LF%>%filter(IDStraftat == "****00" & Bundesland != "Deut")
GG8.2<- ggplot(DS8, aes(x=Jahr, y=AnErfFaelle, colour = Bundesland, group=Bundesland))+ geom_line()+ ggtitle("Entwiklung von Diebstahl in den Bundesländern") +  xlab("Jahr") + ylab("Anzahl der Diebstahle")+theme(plot.title = element_text(size = 14))
GG8.2
...
Save the Dataframes
```{r}
saveRDS(BKA, file="3BKA.rds")
saveRDS(SO, file="3SO.rds")
saveRDS(Data, file="3Data.rds")
```
...
```{r}
#BKA <- readRDS("3BKA.rds")
#SO <- readRDS("3SO.rds")
#Data <- readRDS("3Data.rds")
```
...
path.plot(synth.res = synth.outAnFaellePLACEBO,
           ...
           Ylab = "Anazhl der Straftaten Insgesamt",
           Xlab = "Jahr",
           Legend = c("Baden-Württemberg","synthetic Bayern"),
           Legend.position = "bottomright"
           )
```
...
path.plot(synth.res = synth.outAnVers,
           ...
           Ylab = "Anazhl der Versuche Insgesamt",
           Xlab = "Jahr",
           Legend = c("Bayern","synthetic Bayern"),
           Legend.position = "bottomright"
           )
...
# data preparation
Data <- as.data.frame(Data)
dataprep.outAnVersPLACEBO <-dataprep(
                       foo =Data,
                       predictors = c("BeschAoWoTeil",
                                      "ErwerbMitAkaAbschTeil", 
                                      "ArbeitslosenQuote", 
...
# function
synth.outAnVersPLACEBO <- synth(data.prep.obj = dataprep.outAnVersPLACEBO,method = "BFGS")
synth.outAnVersPLACEBO

#path plot
path.plot(synth.res = synth.outAnVersPLACEBO,
           dataprep.res = dataprep.outAnVersPLACEBO,
           Ylab = "Anazhl der Versuche Insgesamt",
           Xlab = "Jahr",
           Legend = c("Baden-Württemberg","synthetic Bayern"),
           Legend.position = "bottomright"
...
# data preparation
Data <- as.data.frame(Data)
dataprep.outTeilVersPLACEBO <-dataprep(
                       foo =Data,
                       predictors = c("BeschAoWoTeil",
                                      "ErwerbMitAkaAbschTeil", 
                                      "ArbeitslosenQuote", 
...
# function
synth.outTeilVersPLACEBO <- synth(data.prep.obj = dataprep.outTeilVersPLACEBO,method = "BFGS")
synth.outTeilVersPLACEBO

#path plot
path.plot(synth.res = synth.outTeilVersPLACEBO,
           dataprep.res = dataprep.outTeilVersPLACEBO,
           Ylab = "Versuchsanteil Insgesamt",
           Xlab = "Jahr",
           Legend = c("Rheinland-Pfalz","synthetic Bayern"),
           Legend.position = "bottomright"
           )
```
...
path.plot(synth.res = synth.outAnErkl,
           ...
           Ylab = "Anzahl der aufgeklärten Fälle Insgesamt",
           Xlab = "Jahr",
           Legend = c("Bayern","synthetic Bayern"),
           Legend.position = "bottomright"
           )
...
# data preparation
Data <- as.data.frame(Data)
dataprep.outAnAufklPLACEBO <-dataprep(
                       foo =Data,
                       predictors = c("BeschAoWoTeil",
                                      "ErwerbMitAkaAbschTeil", 
                                      "ArbeitslosenQuote", 
...
# function
synth.outAnAufklPLACEBO <- synth(data.prep.obj = dataprep.outAnAufklPLACEBO,method = "BFGS")
synth.outAnAufklPLACEBO

#path plot
path.plot(synth.res = synth.outAnAufklPLACEBO,
           dataprep.res = dataprep.outAnAufklPLACEBO,
           Ylab = "Anzahl der aufgeklärten Fälle Insgesamt",
           Xlab = "Jahr",
           Legend = c("Baden-Württemberg","synthetic Bayern"),
           Legend.position = "bottomright"
...
path.plot(synth.res = synth.outTeilAufk,
           ...
           Ylab = "Aufklärungsquote Insgesamt",
           Xlab = "Jahr",
           Legend = c("Bayern","synthetic Bayern"),
           Legend.position = "bottomright"
           )
...
# data preparation
Data <- as.data.frame(Data)
dataprep.outTeilAufklPLACEBO <-dataprep(
                       foo =Data,
                       predictors = c("BeschAoWoTeil",
                                      "ErwerbMitAkaAbschTeil", 
                                      "ArbeitslosenQuote", 
                       ...
                       controls.identifier = c(9:11,13:16),
                       time.optimize.ssr = 2013:2017,
                       time.plot = 2013:2019
                       )

# function
synth.outTeilAufklPLACEBO <- synth(data.prep.obj = dataprep.outTeilAufklPLACEBO,method = "BFGS")
synth.outTeilAufklPLACEBO

#path plot
path.plot(synth.res = synth.outTeilAufklPLACEBO,
           dataprep.res = dataprep.outTeilAufklPLACEBO,
           Ylab = "Anazhl der Straftaten Insgesamt",
           Xlab = "Jahr",
           Legend = c("Rheinland-Pfalz","synthetic Bayern"),
           Legend.position = "bottomright"
